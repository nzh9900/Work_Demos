...
public class StreamingDemoConnect {
    ...
    public static void main(String[] args) throws Exception {
        ...
        SingleOutputStreamOperator<String> text2_str = text2.map((MapFunction<Long, String>) value -> "str_" + value);

        ConnectedStreams<Long, String> connectStream = text1.connect(text2_str);

        SingleOutputStreamOperator<Object> result = connectStream.map(new CoMapFunction<>() {
            @Override
            public Object map1(Long value) {
                return value;
            }
